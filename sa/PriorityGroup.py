...
class GroupModel():
    ...
    def __init__(self, topology_data, streams_data, idx_priority_group, queue_list, cycle_time, n_neighbours,
                 bandwidth = 1000e6, export_name = None, injection_cycles=0):
        ...
        self.n_neighbours = 5
        self.injection_cycles = int(injection_cycles//self.k_priority)+1 if injection_cycles else 0
    ...
    def GenerateNeighbour(self):
        ...
        new_value  = choice([i for i in range(bot, top) if i not in [assignment[chosen_index]]])
    ...
    def generate_graph(self, topology):
        ...
        print("priority_group:", self.k_priority)
    ...
    def export_full_routes_and_schedules2(self, sol = None, scheduled_line=''):
    ...
    def export_full_routes_and_schedules_csv(self, sol=None, misscheduled_idx=[], convergence=None):
        ...
        with open("results/export_log_" + self.export_name + ".csv", 'a+') as f:
            for idx, (route, stream) in enumerate(zip(sol, self.space)):
                ...
                path_str = '|'.join(['|'.join([el[0] + '-' + el[3], el[1], el[2].strip('q')]) for el in
                                     zip(path_ids[1:], link_ids[1:], queues_chosen, port_ids[1:])])
                path_str = path_ids[0] + '|' + link_ids[0] + '|' + path_str + '|' + path_ids[-1]
    ...
    def export_full_routes_and_schedules_csv2(self, sol = None, misscheduled_idx=[] ):
        ...
        with open("results/export_log_"+self.export_name+".csv", 'a+') as f:
            for idx, (route, stream) in enumerate(zip(sol, self.space)):
                ...
                cycle_injection = route[-1]
                path_ids = [self.graph.all_vertices[el].id for el in path]
                link_ids = [self.graph.all_links[el].name for el in path]
                port_ids = [self.graph.all_links[el].p[0][:] for el in path]
                current_cycle = -1+cycle_injection
                priority_group = -1 + stream[0].k
                queues = self.queue_list[priority_group]
