...
from SimulatedAnnealing import E2E_objective, reset_global_cache, simulated_annealing

LINK_BANDWIDTH = 1000e6
...
def run_network_model(all_streams_data, topology_data, export_name=None, is_use_SA=False, cycle_times=None):
    ...
    for i in range(delta_amount_flows1):
        if i+1 > abs(delta_amount_flows3) and streams_data2:
            ...
            if streams_data3:
                #streams_data1.append(streams_data3.pop())
    ...
    if not is_use_SA:
        ...
        best_individual1, _ = simulated_annealing(PG1) #genetic_algorithm(PG1)
        best_individual2, _ = simulated_annealing(PG2) #genetic_algorithm(PG2)
        best_individual3, _ = simulated_annealing(PG3) #genetic_algorithm(PG3)
...
if __name__ == '__main__':
    for i in range(1,2):
        ...
        topo_filepath = "TC"+str(i)+"_topo.txt"
        flows_filepath = "TC"+str(i)+"_flows.txt"
